...
describe('computeFixedPoints()', () => {
  ...
  it('should stop when a collision with a ball occurs but the bottom wall has not been cleared and the ball bounces up', () => {
    const ball = { radius: 0.025, angle: Math.PI / 2, x: 0.8, y: -0.1, velocity: 1, acceleration: -0.4 };
    // At x + cos(45) * radius * 2, the ball should bounce at 180°. Shift ball1 a bit to the left to make the ball bounce down.
    const ball1 = { counter: 3, radius: 0.025, x: 0.8 + Math.cos(Math.PI / 4) * 0.025 * 2 - 0.001, y: 0.025 };
    const fixedPoints = [...computeFixedPoints(ball, [ball1], { epsilon: 1e-5 })];
    assert.sameDeepOrderedMembers(fixedPoints, [
      { t: 0, x: ball.x, y: ball.y, angle: ball.angle, velocity: ball.velocity, acceleration: ball.acceleration, obstacles: [] },
      {
  ...
  it('should not stop when a collision with a ball occurs but the bottom wall has not been cleared and the ball bounces down', () => {
    const ball = { radius: 0.025, angle: Math.PI / 2, x: 0.8, y: -0.1, velocity: 1, acceleration: -0.4 };
    // At x + cos(45) * radius * 2, the ball should bounce at 180°. Shift ball1 a bit to the right to make the ball bounce up.
    const ball1 = { counter: 3, radius: 0.025, x: 0.8 + Math.cos(Math.PI / 4) * 0.025 * 2 + 0.001, y: 0.025 };
    const fixedPoints = [...computeFixedPoints(ball, [ball1], { epsilon: 1e-5 })];
    assert.sameDeepOrderedMembers(fixedPoints, [
      { t: 0, x: ball.x, y: ball.y, angle: ball.angle, velocity: ball.velocity, acceleration: ball.acceleration, obstacles: [] },
